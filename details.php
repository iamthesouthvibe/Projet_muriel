...
if (isset($_GET['room'])) {
  ...
  if (isset($_POST['checkin2'])) {
    if (!empty($_POST['fullname2']) && !empty($_POST['title']) && !empty($_POST['comment']) && !empty($_POST['date_c'])) {
      $fn = $_POST['fullname2'];
      $comment = $_POST['comment'];
      $title = $_POST['title'];
      $date_c = $_POST['date_c'];
      
      $insert = "INSERT INTO `comments` (`id`, `fullname`, `title_c`, `date_c` ,`comment`, `id_rooms`) VALUES (NULL, '$fn', '$title', '$date_c' ,'$comment', '$roomID')";
      $save = $db->query($insert);
    } else {
      echo '<br /> All fields are required!';
    }
...
<div class="container">
  <?php while ($room = mysqli_fetch_assoc($select)) : ?>
    ...
    <?php 
    $select2 = $db->query("SELECT * FROM `comments` WHERE id_rooms='{$roomID}'");
    while ($comments = mysqli_fetch_assoc($select2)) {
      ?>
      <h5><?php echo $comments['fullname']; ?></h5>
      <h3><?php echo $comments['title_c']; ?></h3>
      <h6><?php echo $comments['date_c']; ?></h6>
    ...
    <form action="details.php?room=<?= $roomID ?>" method="POST">
      ...
      <label for="title">Title of comment</label>
      <input type="text" name="title">

      <label for="date_c">Date de séjour</label>
      ...
      <div class="col-md-12">
          <label></label>
          <input type="submit" class="form-control btn btn-primary" value="Add comment" name="checkin2" <?= ($room['rooms'] <= 0) ? 'disabled' : ''; ?>>
        </div>
    </form>
  <?php endwhile; ?>
</div>
